...
import {
  CurrentSeasonIsEdited,
  CurrentSeasonIsLoading,
  SeasonalWithDetails,
  SeasonalTableItemProps,
} from '@/lib/types'
...
function ConfirmModal({
  ...
  if (confirmModal === 'DELETEALL') {
    return (
      <Dialog fullWidth maxWidth='xs' open={!!confirmModal} onClose={() => setConfirmModal('')}>
        <div className='p-2'>
          <DialogTitle fontSize='large'>Confirm delete all entries?</DialogTitle>
          <DialogActions>
            <Button onClick={handleDeleteAll} variant='outlined'>
              Yes
            </Button>
            <Button onClick={() => setConfirmModal('')} color='error'>
              No
            </Button>
          </DialogActions>
        </div>
...
function ChangeStatusAllModal({
  ...
  return (
    <Dialog fullWidth maxWidth='xs' open={editModal} onClose={() => setEditModal(false)}>
...
function SeasonalTableItem({ props }: SeasonalTableItemProps) {
  ...
  return (
    ...
    >
      ...
      >
        <span
          style={{
            margin: isEdited == `seasonal-title_${item.title}_${item.order}` ? 0 : '0 1rem',
          }}
      ...
      <div
        ...
        className={`relative flex items-center justify-center rounded-e-md lg:rounded-none ${determineStatus(
          item.status
        )}`}
      >
  ...
  function handleMenuClick(e: BaseSyntheticEvent, item: SeasonalWithDetails) {
    const { top, left } = e.target.getBoundingClientRect()
  ...
  function EditStatus({ order, ogvalue }: { order: number; ogvalue: string }) {
    async function handleSubmit(event: BaseSyntheticEvent) {
      event.preventDefault()
      const currentlyProcessedEdit = isEditedRef.current
    ...
    return (
      <div
        style={{
          backgroundColor: isLoadingEditForm.includes(`seasonal-status_${order}`)
            ? 'black'
            : 'unset',
        }}
      ...
      >
        <div
          style={{
            ...
            pointerEvents: isLoadingEditForm.includes(`seasonal-status_${order}`)
              ? 'none'
              : 'unset',
          }}
          className='w-full'
