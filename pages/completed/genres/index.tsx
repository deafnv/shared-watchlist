...
export default function Genres({ response }: { response: Genres[] }) {
  ...
  const [advancedSearchResult, setAdvancedSearchResult] = useState<CompletedWithGenre[] | null>(
    null
  )
...
function AdvancedSearchModal({
  ...
  async function handleSubmit(e: BaseSyntheticEvent) {
    ...
    const { data } = await axios.get(
      `${process.env.NEXT_PUBLIC_UPDATE_URL}/table/completedsbygenreid`,
      {
        params: {
          id: arrIncluded,
        },
  ...
  return (
    <Dialog fullWidth maxWidth='lg' open={advancedSearch} onClose={() => setAdvancedSearch(false)}>
      <div className='fixed flex flex-col items-center gap-4 h-[85dvh] w-[45rem] max-w-[95%] px-10 py-6 bg-background rounded-md shadow-md shadow-black drop-shadow-md top-1/2 left-1/2 -translate-x-1/2 -translate-y-[45%]'>
        ...
        <Button form='advanced-search' type='submit' variant='outlined' size='large'>
          Search
        </Button>
      </div>
    </Dialog>
...
function AdvancedSearchTable({
  ...
  return (
    <div className='p-2 w-full bg-primary-foreground rounded-md'>
      <div className='grid grid-cols-[2fr_1fr_1fr] border-b'>
        <div className='flex items-center justify-center p-2 pt-1 text-lg text-center font-semibold'>
          Title
        </div>
        <div
          onClick={() => handleSort('rating1')}
          className='flex items-center justify-center p-2 pt-1 text-lg text-center font-semibold cursor-pointer'
        >
        ...
        {advancedSearchResult?.length ? (
          advancedSearchResult.map((item: any, index: number) => {
            return (
              <li
                key={index}
                className='grid grid-cols-[2fr_1fr_1fr] p-0 text-center rounded-md hover:bg-primary-accent'
              >
          ...
          <span className='text-lg my-auto self-center'>No results found.</span>
        )}
      </ul>
    </div>
  )
