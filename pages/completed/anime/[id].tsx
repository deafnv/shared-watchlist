  ...
  return (
    <>
      ...
      <main className='flex flex-col items-center justify-center mx-auto mb-16 px-12 py-6 md:w-3/5 sm:w-full'>
        ...
        <h5 className='text-lg text-center'>{response?.[0].CompletedDetails.mal_alternative_title}</h5>
        <Image src={response?.[0].CompletedDetails.image_url} height={380} width={280} alt='Art' className='my-4' />
        ...
        <div className='flex gap-16'>
          ...
          <div className='flex flex-col px-8 py-4 border-[1px] border-white'>
            <h5 className='self-center mb-4 text-xl font-semibold link'><Link href={`https://myanimelist.net/anime/${response?.[0].CompletedDetails.mal_id}`} target='_blank'>MyAnimeList</Link></h5>
            <div className='flex flex-grow items-center mb-6 gap-16'>
              <div className='flex flex-col items-center whitespace-nowrap'>
                <h6 className='mb-2 font-semibold text-lg'>Start</h6>
                <span>{new Date(response?.[0].CompletedDetails.start_date).toLocaleDateString('en-US', {year: 'numeric', month: 'long', day: 'numeric'})}</span>
              </div>
              <div className='flex flex-col items-center whitespace-nowrap'>
                <h6 className='mb-2 font-semibold text-lg'>End</h6>
                <span>{new Date(response?.[0].CompletedDetails.end_date).toLocaleDateString('en-US', {year: 'numeric', month: 'long', day: 'numeric'})}</span>
              </div>
            </div>
          </div>
        </div>
