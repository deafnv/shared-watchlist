...
export default function CompletedErrors() {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const levenDistance = data?.map((item) => {
        const distance = levenshtein(item.title!, (item.CompletedDetails as Database['public']['Tables']['CompletedDetails']['Row']).mal_title!)
        if (distance! > 5) {
          return {
            id: item.id,
            mal_id: (item.CompletedDetails as Database['public']['Tables']['CompletedDetails']['Row']).mal_id,
            entryTitle: item.title,
            retrievedTitle: (item.CompletedDetails as Database['public']['Tables']['CompletedDetails']['Row']).mal_title,
            distance: distance
          }
        }
        return null;
      ...
      <div>
        ...
        <div className="fixed flex flex-col items-center h-[30rem] w-[50rem] px-10 py-6 bg-gray-700 rounded-md shadow-md shadow-black drop-shadow-md border-4 border-black top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 modal">
          ...
          <form onSubmit={handleChange} className='flex flex-col items-center absolute top-[40%] w-3/5'>
            ...
            <Link href={`https://myanimelist.net/anime.php?q=${changed?.retrievedTitle?.substring(0, 64)}`} target='_blank' className='text-lg link'>Search for anime title</Link>
          </form>
        </div>
      </div>
    )
